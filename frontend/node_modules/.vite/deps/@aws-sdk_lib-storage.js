...
import { H as toEndpointV1, d as getEndpointFromInstructions } from "./SignatureV4-BBiomiEu.js";
import { Lr as CompleteMultipartUploadCommand, Mr as CreateMultipartUploadCommand, Rr as AbortMultipartUploadCommand, Rt as UploadPartCommand, Zt as PutObjectCommand, p as ChecksumAlgorithm, qt as PutObjectTaggingCommand } from "./dist-es-XJoMMF6y.js";
import { a as extendedEncodeURIComponent } from "./HttpInterceptingShapeSerializer-Lr2NmiMS.js";
...
//#endregion
//#region node_modules/@aws-sdk/lib-storage/dist-es/runtimeConfig.shared.js
var import_events = require_events();
var import_buffer = require_buffer();
//#endregion
//#region node_modules/@aws-sdk/lib-storage/dist-es/runtimeConfig.browser.js
var runtimeConfig = {
	lstatSync: () => {},
	isFileReadStream(f) {
		return false;
...
//#endregion
//#region node_modules/@aws-sdk/lib-storage/dist-es/byteLength.js
var byteLength = (input) => {
	if (input == null) return 0;
	if (typeof input === "string") return import_buffer.Buffer.byteLength(input);
...
//#endregion
//#region node_modules/@aws-sdk/lib-storage/dist-es/byteLengthSource.js
...
var import_stream_browserify = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
async function* getDataReadable(data) {
	for await (const chunk of data) if (import_buffer.Buffer.isBuffer(chunk) || chunk instanceof Uint8Array) yield chunk;
	else yield import_buffer.Buffer.from(chunk);
}
//#endregion
...
async function* getDataReadableStream(data) {
	...
	try {
		while (true) {
			...
			if (done) return;
			if (import_buffer.Buffer.isBuffer(value) || value instanceof Uint8Array) yield value;
			else yield import_buffer.Buffer.from(value);
		}
	} catch (e) {
		throw e;
	} finally {
...
//#endregion
//#region node_modules/@aws-sdk/lib-storage/dist-es/chunker.js
...
//#endregion
//#region node_modules/@aws-sdk/lib-storage/dist-es/Upload.js
var Upload = class Upload extends import_events.EventEmitter {
	...
	constructor(options) {
		...
		if (this.totalBytes !== void 0) this.expectedPartsCount = Math.ceil(this.totalBytes / this.partSize);
	...
	async __doConcurrentUpload(dataFeeder) {
		for await (const dataPart of dataFeeder) {
			...
			const uploadEventListener = (event, request) => {
				if ((Number(request.query["partNumber"]) || -1) !== dataPart.partNumber) return;
	...
	async __doMultipartUpload() {
		...
		if (this.abortController.signal.aborted) {
			await this.markUploadAsAborted();
			throw Object.assign(/* @__PURE__ */ new Error("Upload aborted."), { name: "AbortError" });
		}
		let result;
		if (this.isMultiPart) {
			const { expectedPartsCount, uploadedParts, totalBytes, totalBytesSource } = this;
			if (totalBytes !== void 0 && expectedPartsCount !== void 0 && uploadedParts.length !== expectedPartsCount) throw new Error(`Expected ${expectedPartsCount} part(s) but uploaded ${uploadedParts.length} part(s).
...
`);
	...
	async __abortTimeout(abortSignal) {
		return new Promise((resolve, reject) => {
			abortSignal.onabort = () => {
				const abortError = /* @__PURE__ */ new Error("Upload aborted.");
	...
	__validateUploadPart(dataPart) {
		...
		if (actualPartSize === void 0) throw new Error(`A dataPart was generated without a measurable data chunk size for part number ${dataPart.partNumber}`);
