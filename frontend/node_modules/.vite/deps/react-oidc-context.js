...
function useAutoSignin({ signinMethod = "signinRedirect", signinArgs } = {}) {
	const auth = useAuth();
	const [hasTriedSignin, setHasTriedSignin] = import_react.useState(false);
	const shouldAttemptSignin = import_react.useMemo(() => !hasAuthParams() && !auth.isAuthenticated && !auth.activeNavigator && !auth.isLoading && !hasTriedSignin, [
...
var withAuthenticationRequired = (Component, options = {}) => {
	const { OnRedirecting = () => /* @__PURE__ */ import_react.createElement(import_react.Fragment, null), onBeforeSignin, signinRedirectArgs } = options;
	const displayName = `withAuthenticationRequired(${Component.displayName || Component.name})`;
	const C = (props) => {
		const auth = useAuth();
		import_react.useEffect(() => {
			if (hasAuthParams() || auth.isLoading || auth.activeNavigator || auth.isAuthenticated) return;
			(async () => {
		...
		return auth.isAuthenticated ? /* @__PURE__ */ import_react.createElement(Component, { ...props }) : OnRedirecting();
	};
	C.displayName = displayName;
	return C;
};
//#endregion
export { AuthContext, AuthProvider, hasAuthParams, useAuth, useAutoSignin, withAuth, withAuthenticationRequired };
