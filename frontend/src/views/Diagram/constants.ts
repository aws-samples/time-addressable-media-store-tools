...
const createFormatIcon = (pathData: string) =>
  createSvgDataUri(18, 18, pathData);
const createContainerBadge = () =>
  createSvgDataUri(
    12,
    12,
...
const createFormatStyles = (
  ...
  return {
    ...
    style: {
      backgroundOffsetX: withContainer
        ? [iconOffset.x, iconOffset.x]
        : [iconOffset.x],
      backgroundOffsetY: withContainer
        ? [`-${iconOffset.y}`, iconOffset.y]
        : [iconOffset.y],
      backgroundImage: withContainer
        ? [createContainerBadge(), formatIcon]
        : [formatIcon],
    },
  };
};
...
export const buildStylesheet = () =>
  [
    ...
    ...Object.entries(styles.nodes).map(([type, props]) => ({
      selector: `node.${type}`,
      style: props,
    })),
    // Node format specific styling
    ...Object.entries(svgPaths).flatMap(([format, d]) => [
      createFormatStyles(format, d, false),
      createFormatStyles(format, d, true),
    ]),
    // Edge type specific styling
    ...Object.entries(styles.edges).map(([type, props]) => ({
      selector: `edge.${type}`,
      style: props,
    })),
  ] as StylesheetJson;
