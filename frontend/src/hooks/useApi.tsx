...
export const useApi = () => {
  ...
  return useMemo(() => {
    const makeRequest = async <T = unknown>(method: string, path: string, options: RequestOptions = {}): Promise<ApiResponse<T>> => {
    ...
    return {
      get: <T = unknown>(path: string, options: RequestOptions = {}) => makeRequest<T>("GET", path, options),
